import type { TeamSeed } from "./types";
import { createTeamLogoDataUri } from "./team-logo";

export const TEAM_SEEDS: TeamSeed[] = [
  {
    id: "csk",
    name: "Chennai Super Kings",
    shortName: "CSK",
    logoUrl: createTeamLogoDataUri("CSK", "#FFFF00"),
    primaryColor: "#FFFF00",
    captain: "Ruturaj Gaikwad",
    coach: "Stephen Fleming",
    venue: "M. A. Chidambaram Stadium, Chennai",
    owner: "Chennai Super Kings Cricket Limited",
    titles: [2010, 2011, 2018, 2021, 2023],
    lastYearRank: 10,
    historyRankings: [
      { year: 2025, rank: 10 },
      { year: 2024, rank: 5 },
      { year: 2023, rank: 1 },
      { year: 2022, rank: 9 },
      { year: 2021, rank: 1 },
      { year: 2020, rank: 7 },
    ],
  },
  {
    id: "dc",
    name: "Delhi Capitals",
    shortName: "DC",
    logoUrl: createTeamLogoDataUri("DC", "#00008B"),
    primaryColor: "#00008B",
    captain: "Axar Patel",
    coach: "Hemang Badani",
    venue: "Arun Jaitley Stadium, Delhi",
    owner: "GMR Sports Pvt. Ltd. & JSW Sports Pvt. Ltd.",
    titles: [],
    lastYearRank: 5,
    historyRankings: [
      { year: 2025, rank: 5 },
      { year: 2024, rank: 6 },
      { year: 2023, rank: 9 },
      { year: 2022, rank: 5 },
      { year: 2021, rank: 3 },
      { year: 2020, rank: 2 },
    ],
  },
  {
    id: "gt",
    name: "Gujarat Titans",
    shortName: "GT",
    logoUrl: createTeamLogoDataUri("GT", "#2D3D73"),
    primaryColor: "#2D3D73",
    captain: "Shubman Gill",
    coach: "Ashish Nehra",
    venue: "Narendra Modi Stadium, Ahmedabad",
    owner: "Irelia Sports India Private Limited",
    titles: [2022],
    lastYearRank: 3,
    historyRankings: [
      { year: 2025, rank: 3 },
      { year: 2024, rank: 8 },
      { year: 2023, rank: 2 },
      { year: 2022, rank: 1 },
    ],
  },
  {
    id: "kkr",
    name: "Kolkata Knight Riders",
    shortName: "KKR",
    logoUrl: createTeamLogoDataUri("KKR", "#3A225D"),
    primaryColor: "#3A225D",
    captain: "Ajinkya Rahane",
    coach: "Abhishek Nayar",
    venue: "Eden Gardens, Kolkata",
    owner: "Knight Riders Sports Private Limited",
    titles: [2012, 2014, 2024],
    lastYearRank: 8,
    historyRankings: [
      { year: 2025, rank: 8 },
      { year: 2024, rank: 1 },
      { year: 2023, rank: 7 },
      { year: 2022, rank: 7 },
      { year: 2021, rank: 2 },
      { year: 2020, rank: 5 },
    ],
  },
  {
    id: "lsg",
    name: "Lucknow Super Giants",
    shortName: "LSG",
    logoUrl: createTeamLogoDataUri("LSG", "#0057E2"),
    primaryColor: "#0057E2",
    captain: "Rishabh Pant",
    coach: "Justin Langer",
    venue: "Ekana Cricket Stadium, Lucknow",
    owner: "RPSG Group",
    titles: [],
    lastYearRank: 7,
    historyRankings: [
      { year: 2025, rank: 7 },
      { year: 2024, rank: 7 },
      { year: 2023, rank: 3 },
      { year: 2022, rank: 3 },
    ],
  },
  {
    id: "mi",
    name: "Mumbai Indians",
    shortName: "MI",
    logoUrl: createTeamLogoDataUri("MI", "#004BA0"),
    primaryColor: "#004BA0",
    captain: "Hardik Pandya",
    coach: "Mahela Jayawardene",
    venue: "Wankhede Stadium, Mumbai",
    owner: "Indiawin Sports Pvt. Ltd.",
    titles: [2013, 2015, 2017, 2019, 2020],
    lastYearRank: 4,
    historyRankings: [
      { year: 2025, rank: 4 },
      { year: 2024, rank: 10 },
      { year: 2023, rank: 4 },
      { year: 2022, rank: 10 },
      { year: 2021, rank: 5 },
      { year: 2020, rank: 1 },
    ],
  },
  {
    id: "pbks",
    name: "Punjab Kings",
    shortName: "PBKS",
    logoUrl: createTeamLogoDataUri("PBKS", "#D71920"),
    primaryColor: "#D71920",
    captain: "Shreyas Iyer",
    coach: "Ricky Ponting",
    venue: "New PCA Stadium, New Chandigarh",
    owner: "KPH Dream Cricket Private Limited",
    titles: [],
    lastYearRank: 1,
    historyRankings: [
      { year: 2025, rank: 2 },
      { year: 2024, rank: 9 },
      { year: 2023, rank: 8 },
      { year: 2022, rank: 6 },
      { year: 2021, rank: 6 },
      { year: 2020, rank: 6 },
    ],
  },
  {
    id: "rr",
    name: "Rajasthan Royals",
    shortName: "RR",
    logoUrl: createTeamLogoDataUri("RR", "#EA1A85"),
    primaryColor: "#EA1A85",
    captain: "Riyan Parag",
    coach: "Rahul Dravid",
    venue: "Sawai Mansingh Stadium, Jaipur",
    owner: "Royal Multisport Private Limited",
    titles: [2008],
    lastYearRank: 9,
    historyRankings: [
      { year: 2025, rank: 9 },
      { year: 2024, rank: 3 },
      { year: 2023, rank: 5 },
      { year: 2022, rank: 2 },
      { year: 2021, rank: 7 },
      { year: 2020, rank: 8 },
    ],
  },
  {
    id: "rcb",
    name: "Royal Challengers Bengaluru",
    shortName: "RCB",
    logoUrl: createTeamLogoDataUri("RCB", "#EC1C24"),
    primaryColor: "#EC1C24",
    captain: "Rajat Patidar",
    coach: "Andy Flower",
    venue: "M. Chinnaswamy Stadium, Bengaluru",
    owner: "Royal Challengers Sports Private Ltd",
    titles: [2025],
    lastYearRank: 2,
    historyRankings: [
      { year: 2025, rank: 1 },
      { year: 2024, rank: 4 },
      { year: 2023, rank: 6 },
      { year: 2022, rank: 4 },
      { year: 2021, rank: 4 },
      { year: 2020, rank: 4 },
    ],
  },
  {
    id: "srh",
    name: "Sunrisers Hyderabad",
    shortName: "SRH",
    logoUrl: createTeamLogoDataUri("SRH", "#FF822A"),
    primaryColor: "#FF822A",
    captain: "Pat Cummins",
    coach: "Daniel Vettori",
    venue: "Rajiv Gandhi Intl. Cricket Stadium, Hyderabad",
    owner: "Sun TV Network Limited",
    titles: [2016],
    lastYearRank: 6,
    historyRankings: [
      { year: 2025, rank: 6 },
      { year: 2024, rank: 2 },
      { year: 2023, rank: 10 },
      { year: 2022, rank: 8 },
      { year: 2021, rank: 8 },
      { year: 2020, rank: 3 },
    ],
  },
];
